...
const tsxContent = fs.readFileSync(path.join(__dirname, 'app/page.tsx.part'), 'utf-8');
const vueContent = fs.readFileSync(path.join(__dirname, '../src/pages/HomePage.vue'), 'utf-8');

const styleMatch = vueContent.match(/<style scoped>([\s\S]*?)<\/style>/g);
let cssContent = '';
if (styleMatch) {
  // combine all style blocks just in case
...
fs.writeFileSync(path.join(__dirname, 'app/page.tsx'), finalTsx, 'utf-8');
